...
An MCP server exposing real, semantic Java code-intelligence tools — `java_definition`, `java_references`, `java_hover`, `java_implementation`, `java_document_symbols`, `java_workspace_symbols`, `java_diagnostics` — by spawning and driving a real [jdtls](https://github.com/eclipse-jdtls/eclipse.jdt.ls) (Eclipse JDT Language Server) process over its native LSP stdio protocol. Built directly against `@modelcontextprotocol/sdk`, same hand-rolled pattern as `mcp/oracle/`/`mcp/loki/`. Speaks MCP over Streamable HTTP as a persistent process opencode connects to (`type: "remote"`), same shape as those two.
...
- **One persistent jdtls process for the server's whole lifetime, not one per request.** Unlike `mcp/oracle`'s/`mcp/loki`'s deliberately-stateless per-request design, LSP is a genuinely stateful session — project indexing alone takes real time, and jdtls doesn't support concurrent instances against the same `-data` directory. `server.js` keeps one `LspClient` singleton (lazily started on the first tool call) at module scope; the outer MCP/HTTP layer is still stateless-per-request (fresh `Server`/transport pair per call, same as `mcp/oracle`/`mcp/loki`) — those are two independent layers, and only the inner one needed to change.
- **`lsp-client.js` is duplicated into `mcp/spring-lsp/`, not shared via a package dependency.** Both packages need the same framing/handshake/sync engine, verbatim. Rather than introduce a cross-package `file:` dependency (which nothing else under `mcp/` does — each package there is independently installable), the file is copied. See `mcp/spring-lsp/README.md` for the same note from that side.
...
Copy `.env.example` to `.env` and fill in real values, or set them however the process supervisor that starts this server is configured (see `mcp/loki/README.md`'s Run section for supervisor options — same reasoning applies here).
...
- `JAVA_LSP_MCP_PORT` — port to listen on (optional, defaults to `8092`, the next free port after `mcp/loki`'s `8091`)
...
**Not yet wired into `tests/run-in-container.sh` / the docker/ sandbox** — the sandbox's base image has no JDK or jdtls, and adding a jdtls download to the Dockerfile wasn't done as part of this change (would need explicit sign-off, same as any new download source — see `mcp/TODO.md`). Run `java-lsp.test.mjs` directly on a machine with `jdtls` installed for now.
