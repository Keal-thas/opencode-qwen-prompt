...
- **All development and testing for this repo happens inside the `docker/` sandbox, never directly against the host's own node/opencode/npm install.** Stated explicitly 2026-09-13 when the `tests/` suite (see `tests/README.md`) was added. Run `./tests/run-all.sh` to build the sandbox image and run the whole suite; iterate with `docker compose -f docker/docker-compose.yml run --rm opencode-dev bash tests/run-in-container.sh` for the faster in-container-only subset. The one narrow exception is `tests/integration/docker-prompt-override.test.sh`, which necessarily runs on the host because it's the thing invoking `docker run` in the first place — same as every other `docker compose ...` command already documented in `docker/docker-notes.md` — but everything it actually asserts on still executes inside a disposable container, never against the host's own opencode/node.
...
- **The CLI's "can't run a subagent directly" restriction is a CLI-argument-parsing thing, not a server-side rule — the HTTP/SDK layer has no such restriction, but that doesn't make a subagent a safe substitute for a real permission-layer guarantee.** Found 2026-09-14 while rewriting `module-analysis/analyze-modules.mjs` to use `@opencode-ai/sdk` instead of shelling out to `opencode run --agent <name>`. Verified live against a real opencode server (via `createOpencode()`) with a real DeepSeek-backed model: `client.session.prompt({body: {agent: "explore", ...}})` actually runs as `explore` (`info.mode`/`info.agent` came back `"explore"`, and the model's own reasoning referenced its real "read-only file search specialist" identity) — the CLI's silent fallback to `build` for subagent-only agents doesn't exist at this layer. But dumping `client.app.agents()`'s full permission array for each built-in agent showed `explore` (and `general`) carry only a blanket `{permission:"*", action:"allow"}` with no `edit`-specific rule at all — their "read-only"/tool-scoped framing in the docs is enforced purely by their own system prompt text ("Do not create any files, or run bash commands that modify the user's system state"), the same soft guarantee this file's "bash tool is unrestricted" caveat about `plan` already warned about, not a hard one. `plan`'s permission array, by contrast, has a real `{permission:"edit", pattern:"*", action:"deny"}` entry. So for anything unattended over arbitrary/untrusted third-party code (module-analysis's whole premise), `plan` remains the safer choice specifically *because* of its permission-layer denial — being able to target `explore` directly via the SDK is a genuine new capability, not automatically a safety upgrade over `plan` for this kind of use case.
...
- **`deploy/system-prompt-tools.js` → `.ts` rewrite (2026-09-14): brought the third plugin in line with `plugins/`'s typed-against-the-SDK pattern, but this one ships as a standalone file to an offline machine with no build step, so the loader question needed its own verification, not just a copy of the `plugins/` precedent.** Confirmed `input.model.providerID`/`.id`/`input.sessionID`/`output.system` against the real cached `@opencode-ai/plugin@1.18.30` types inside the docker sandbox (`experimental.chat.system.transform`'s real signature: `input: { sessionID?: string; model: Model }, output: { system: string[] }` — `model` is non-optional, so the old `input.model?.` optional-chaining was unnecessary and got dropped); `tsc --noEmit --strict` passed clean with zero changes needed beyond that, meaning the original hand-typed-by-inference JS was already accessing the right shape. The separate, real question — does opencode's own plugin loader actually load a local `.ts` file at runtime on the target, independent of whatever Node happens to be on that machine — couldn't be fully closed: `opencode debug config` accepts a `.ts` `plugin` path and resolves it into `plugin_origins` without complaint, but a follow-up check (pointing it at a deliberately syntax-broken `.ts` file) proved `debug config` never actually imports the plugin module at all, so that result is weaker evidence than it first looked. A real end-to-end check via `opencode run` (which would genuinely import the plugin to register its hooks) was blocked by an unrelated, reproducible bug — see the next entry. Proceeded with the rename anyway on the strength of `plugins.mdx` documenting local `.ts` plugins as first-class (opencode's plugin loader runs on Bun, which has always had native TS support, unlike Node's version-gated `--experimental-strip-types`) and because this plugin is diagnostic-only (dumps the assembled prompt for manual inspection) — its failure mode if `.ts` loading ever doesn't work on some future opencode version is silent and self-evident (no dump file appears), not a break of the actual prompt override (that's `agent.prompt` in `opencode.json`, unaffected). Worth a real spot-check next time SETUP.md's step 4/6 actually runs against the real target machine.
- **`opencode run` hangs indefinitely right after the `init` bootstrap log line, reproduced with zero plugins configured — a real, unrelated bug in this docker sandbox, discovered while trying to verify the `.ts` plugin rewrite above.** `opencode debug config` and `opencode debug startup` both return fine; `opencode run --model deepseek/deepseek-v4-flash "..."` with `--log-level DEBUG --print-logs` prints the standard ~9-line bootstrap sequence (config loading, LSPs/formatters disabled, `message=init`) and then produces zero further output before a 60-100s `timeout` kills it, confirmed on a container with `DEEPSEEK_API_KEY` present and `api.deepseek.com` independently reachable via `curl`, and confirmed NOT specific to the plugin change (same hang with `"plugin"` omitted from config entirely). Not a stale-lock issue either (checked `~/.config/opencode`, `~/.local/share/opencode`, `~/.local/state/opencode`, `~/.cache/opencode` for lock files after force-killing a hung container — nothing found). Root cause not identified — didn't chase further since it's orthogonal to the Qwen override (which is verified via `opencode debug config`, not `opencode run`) and the docker sandbox's own scripted tests (`./tests/run-all.sh`) don't exercise `opencode run` at all. Worth its own investigation session before relying on `opencode run` for anything in this sandbox.
...
- **This repo's own layout, post-2026-09-13 reorg** (was flat, everything
  ...
  `opencode.json.example`, `system-prompt-tools.ts`,
  ...
  `tests/` (added 2026-09-13) is the automated test suite covering this workspace's feature points — plugin unit tests, config-consistency checks, and integration tests for `analyze-modules.mjs` and the docker sandbox's prompt override; see `tests/README.md` for the full breakdown and `./tests/run-all.sh` to run it.
  `mcp/` (added 2026-09-13) is MCP servers for LAN-internal ops tooling, one subdirectory per server, hand-rolled against the raw SDK the same way `plugins/` was before its TypeScript rewrite — `mcp/oracle/` (full-passthrough, audit hook currently a no-op by explicit decision) is the one built so far, verified end-to-end against a real Oracle instance and wired into `deploy/opencode.json.example`; see `mcp/TODO.md` for what's next (Loki is highest priority).
