...
- **The project directory itself persists via a live bind mount, not container-lifecycle persistence** — `docker-compose.yml` bind-mounts the repo root to `/home/dev/project`: edits to `deploy/system-prompt.txt` on the Mac host show up immediately, no rebuild. `deploy/system-prompt-tools.ts`/`plugins/hook-logger.ts`/`plugins/llm-review-gate.ts` get this too — `docker-entrypoint.sh` copies them fresh from the bind mount into `~/.config/opencode/plugins/` on every start, so source edits take effect on the next `opencode` invocation with no rebuild.
...
**Automatic now, not a manual step.** `docker-entrypoint.sh` (re)generates `~/.config/opencode/opencode.jsonc` fresh on every container start — `agent.build/plan/general.prompt` wired to `/home/dev/project/deploy/system-prompt.txt` (the bind-mounted, live file) — and separately copies `deploy/system-prompt-tools.ts` into `~/.config/opencode/plugins/`, where opencode auto-loads it (see "Plugin dependency pre-warming" above). There's no config volume to hand-edit anymore.
