...
- `analyze-modules.mjs` uses `@opencode-ai/sdk`'s typed `client.session.prompt()` against a real opencode server it starts itself. Verified against both a fake local model provider and a real DeepSeek-backed one; see `tests/integration/analyze-modules.test.mjs` and [docs/lessons-learned.md](../docs/lessons-learned.md) for why `plan` stays the default agent despite `explore` fitting the read-only framing better by name. Untested: `createOpencode()` defaults to port 4096, same as an interactive `opencode` TUI session — running this alongside one on the same machine may collide.
