...
**Not yet wired into `tests/run-in-container.sh` / the docker/ sandbox** — the sandbox's base image has no JDK or jdtls, and adding a jdtls download to the Dockerfile needs explicit sign-off, same as any new download source (see `mcp-servers/TODO.md`). Run `java-lsp.test.mjs` directly on a machine with `jdtls` installed for now.
