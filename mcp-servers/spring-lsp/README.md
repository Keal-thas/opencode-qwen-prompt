...
**Protocol plumbing verified end-to-end against the real, vendored `spring-boot-language-server` 2.5.0-SNAPSHOT** — both manually and by `spring-lsp.test.mjs`: the `initialize` handshake succeeds; the auto-extraction of the vendored tarball works; `.java`/`.properties` files can be opened and synced; `spring_boot_structure`'s real `sts/spring-boot/structure` custom command round-trips cleanly; `spring_diagnostics`/`spring_completion` on a `.properties` file return cleanly without crashing the server. The fuller client-capabilities object in `lsp-client.js` is required for this server; keep the `defaultClientCapabilities()` comment if editing it.
