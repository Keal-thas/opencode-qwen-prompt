...
| [mcp/spring-lsp/vendor/spring-boot-language-server-2.5.0-SNAPSHOT.tar.gz](../mcp/spring-lsp/vendor/spring-boot-language-server-2.5.0-SNAPSHOT.tar.gz) | [mcp/spring-lsp/fetch-spring-boot-language-server.sh](../mcp/spring-lsp/fetch-spring-boot-language-server.sh) | `spring-boot-language-server` isn't published to Maven Central or any registry, only distributed inside VMware's VS Code extension `.vsix` — vendored directly since there's no install-from-registry alternative (see `mcp/spring-lsp/README.md`'s Vendoring section) |
