...
**Deployment (changed 2026-09-15, install mechanism changed 2026-09-16):** the committed `plugins/opencode-hook-plugins-1.0.0.tgz` was packed and drift-tested from the start, but until now nothing actually installed it anywhere — SETUP.md's own deployment walkthrough never referenced it. SETUP.md step 5 now installs it on the target machine by extracting the tarball into opencode's own package cache and referencing it as a bare `opencode-hook-plugins@1.0.0`, purely opt-in (skip it unless specifically wanted) — see `docs/feature-points/02-system-prompt-tools-plugin.md` for the mechanism.
