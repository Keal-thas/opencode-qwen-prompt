...
`SETUP.md` is written to be executed *by an agent* on the target machine (no internet, git-bash on Windows) — copies files in, merges the `agent` config key, optionally installs the viewer plugin, optionally wires in the Oracle MCP server (step 5 - explicitly flagged as not fully supported yet, since `mcp/oracle/`'s npm dependencies aren't vendored for offline use the way `plugins/` are, see `mcp/TODO.md`), verifies, reports back. `SETUP-walkthrough.zh.md` is a separate Chinese human-facing walkthrough of the same steps, not meant to be executed literally.
