...
The repo's original namesake feature. Replaces opencode's built-in `default.txt` system prompt with a custom one (`deploy/system-prompt.txt`) for the `build`/`plan`/`general` agents, via opencode's native `agent.<name>.prompt: "{file:...}"` config field — no plugin needed for the override itself, and nothing model-specific about the mechanism: it works the same regardless of which provider/model is behind the agent. `deploy/opencode.json.example` is the config template; `SETUP.md` walks an agent through installing it on the actual offline Windows/vLLM target machine.
